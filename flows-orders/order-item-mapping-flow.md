...
* Price book entry lookup and assignment for product relationships with currency support
...
| `OrderCurrencyType` | String                      | No       | Currency code for multi-currency support      |
...
* Calls Apex action `movedata__CurrencyCodeComponent` to determine and validate currency
* Processes OrderCurrencyType for multi-currency support
* Ensures proper currency handling for price book entries

**Price Book Entry Lookup:**

* Calls Apex action `GetPriceBookEntryFlowComponent` with currency support
* Passes CurrencyCode, Product2Id, and Pricebook2Id parameters
* Validates price book entry exists before proceeding
* Throws error if no matching price book entry is found
...
* Errors variable collects all error messages
* "Could not find a Pricebook Entry." error for missing price book entries
...
* `movedata__CurrencyCodeComponent` - Apex action for currency code processing
* `GetPriceBookEntryFlowComponent` - Apex action for price book entry retrieval