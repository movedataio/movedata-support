...
**Label:** \[MoveData] General: Campaign - Mapping\
**Type:** Auto-Launched Flow Template\
**API Version:** 49.0\
**Status:** Active

This flow handles the mapping and transformation of campaign data from external platforms into Salesforce Campaign records, with support for Non-Profit Cloud integration.
...
* Non-Profit Cloud integration fields
...
| md\_npc\_pack\_\_Campaign\_URL\_\_c        | URL                | Stores web address for online campaign pages         |
| md\_npc\_pack\_\_Fundraising\_Account\_\_c | Lookup to Account  | Associates campaigns with organizational fundraisers |
...
| `CampaignContact`    | Contact SObject  | Associated contact for Non-Profit Cloud integration |
| `CampaignAccount`    | Account SObject  | Associated account for Non-Profit Cloud integration |
...
| `Config_CampaignIgnoreStartDate`     | Boolean | false   | Skip start date processing |
| `Config_CampaignIgnoreEndDate`       | Boolean | false   | Skip end date processing   |
| `Config_CampaignIgnoreName`          | Boolean | false   | Skip name processing       |
| `Config_CampaignIgnoreDefaultStatus` | Boolean | -       | Skip default status logic  |
| `Config_CampaignIgnoreDefaultType`   | Boolean | -       | Skip default type logic    |
...
1. **Ignore**: If `Config_CampaignIgnoreDefaultType` is true
...
* Can be bypassed with `Config_CampaignIgnoreDefaultStatus`
...
* **End Date**: Converts to local date with configuration-based control
...
### 6. Non-Profit Cloud Integration

When Non-Profit Cloud fields are available:

* Sets `md_npc_pack__Campaign_URL__c` from `PageUrl`
* Sets `md_npc_pack__Fundraising_Account__c` from associated account (when `HasCampaignAccount` is true)
* Uses contact association logic for fundraising contact assignment
...
9. **Non-Profit Cloud Integration**: Set URL and fundraising associations
...
* **Ignore Names**: `Config_CampaignIgnoreName = true` skips all name processing
* **Protection Field**: `movedata__Protect_Name__c` prevents individual record updates

### Date Control

* **Start Date**: `Config_CampaignIgnoreStartDate = true` skips start date processing
* **End Date**: `Config_CampaignIgnoreEndDate = true` skips end date processing

### Type and Status Control

* **Default Type**: `Config_CampaignIgnoreDefaultType = true` skips type assignment
* **Default Status**: `Config_CampaignIgnoreDefaultStatus = true` skips status mapping
